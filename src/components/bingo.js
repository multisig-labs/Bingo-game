...
const Bingo = () => {
  ...
  const [clickedTiles, setClickedTiles] = useState(() => {
    ...
    return savedClickedTiles
      ? JSON.parse(savedClickedTiles)
      : Array(3)
          .fill(null)
  ...
  const handleTileClick = (rowIndex, colIndex) => {
    ...
    const newClickedTiles = clickedTiles.map((row, rldx) =>
      row.map((clicked, cldx) =>
        rldx === rowIndex && cldx === colIndex ? true : clicked
      )
    );
    ...
    const isCardFullyClicked = newClickedTiles.every((row) =>
  ...
  return (
    <div
      className={`bingo container ${
        isModalOpen || isFreeModalOpen ? "blur" : ""
      }`}
    >
      <HeaderImg />
            ...
            return (
              <div
                ...
                className={`bingo-tile ${
                  clickedTiles[rowIndex][colIndex] ? "clicked" : ""
